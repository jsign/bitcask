	...
	t.Run("truncated", func(t *testing.T) {
		...
		for i := range table {
			t.Run(table[i].name, func(t *testing.T) {
				...
				if err := read(bf, art.New(), 1024, 1024); errors.Cause(err) != table[i].err {
	...
	t.Run("overflow", func(t *testing.T) {
		...
		for i := range table {
			t.Run(table[i].name, func(t *testing.T) {
				...
				if err := read(bf, art.New(), table[i].maxKeySize, table[i].maxValueSize); errors.Cause(err) != table[i].err {
					t.Fatalf("expected %v, got %v", table[i].err, err)
				}
			})
		}
